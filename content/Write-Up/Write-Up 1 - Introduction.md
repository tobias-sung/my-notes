...
With the main features completed, I added two extra ones to make the system more convenient. The first was to add an [[Write Up 6 - OLED screen|small OLED screen]] that printed out console messages. The second was to have the Pico W read configuration information (such as the Wi-Fi SSID/password and the server IP/domain name) from a [[text file]].  
