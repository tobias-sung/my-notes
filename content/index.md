---
title: Welcome!
---
Hello! My name is Tobias, and I'm an aspiriring software engineer (click [[📜 About Me|here]] if you'd like to know more about me). 




<div class="nav-buttons">
<button class="nav-button" id="prevBtn"><img src="Images/left-arrow.png"></button>
<p style="justify-content: center; font-family: var(--headerFont); font-size:20; display: flex;"><b>Projects</b></p>
<button class="nav-button" id="nextBtn"><img width=100% src="Images/right-arrow.png"></button>
</div>
<div class="carousel-container">
<div class="carousel-wrapper" id="carousel">
<div class="carousel-track" id="carouselTrack">
<div class="carousel-item project-carousel-item">
<p class="carousel-caption">Modifying VL535CX Driver</br>to Use Software I2C (PIO)</p>
<a href="./Project-Portos/9 - Modifying-VL535CX-Driver-to-Use-Software-I2C-(PIO)" class="carousel-link" target="_top">
<img src="../Images/DRIVER.png" alt="Project 1" class="carousel-image">
</a>
</div>
<div class="carousel-item project-carousel-item">
<p class="carousel-caption">MSC Text File Configuration</p>
<a href="./Project-Portos/7 - Configuration File" class="carousel-link" target="_top">
<img src="../Images/CONFIG.png" alt="Project 2" class="carousel-image">
</a>
</div>
<div class="carousel-item project-carousel-item">
<p class="carousel-caption">SD Card Debug Logger</p>
<a href="./Project-Portos/10 - Pico W SD Card Debug Logger" class="carousel-link" target="_top">
<img src="../Images/SD.png" alt="Project 2" class="carousel-image">
</a>
</div>
<div class="carousel-item project-carousel-item">
<p class="carousel-caption">Battery Display</p>
<a href="8 - Battery Display on the Pico W" class="carousel-link" target="_top">
<img src="../Images/Power Monitor 1.gif" alt="Project 2" class="carousel-image">
</a>
</div>
</div>
</div>
</div>

This website is a place for me to keep notes on all the projects I've worked on. It's a bit bare-bones in terms of design or artistic flair, but it does its job!

I strongly believe in the importance of personal documentation when programming. As the famous saying goes, *"When I wrote this code, only God and I understood what I did. Now only God knows."* Hence the need to **write things down**!
****
In an effort to improve my technical communication skills, I've also tried my hand at making some educational videos using animation tools like [MotionCanvas](https://motioncanvas.io). Here are some of my video works:

<div class="nav-buttons">
<button class="nav-button" id="prevBtn2"><img src="Images/left-arrow.png"></button>
<p style="justify-content: center; font-family: var(--headerFont); font-size:20; display: flex;"><b>Videos</b></p>
<button class="nav-button" id="nextBtn2"><img width=100% src="Images/right-arrow.png"></button>
</div>
<div class="carousel-container">
<div class="carousel-wrapper" id="carousel">
<div class="carousel-track" id="carouselTrack2">
<div class="carousel-item video-carousel-item">
<p class="carousel-caption">Raspberry Pi Pico</br>Programmable I/O (A Visual Explanation)</p>
<iframe width="500" height="315" src="https://www.youtube.com/embed/iynYiE6B01w" frameborder="0" allow="accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
</div>
<div class="carousel-item video-carousel-item">
<p class="carousel-caption">How Ring Buffers Work</br>(The Playground Roundabout Analogy)</p>
<iframe width="500" height="315" src="https://www.youtube.com/embed/jMgYWifqxLE" frameborder="0" allow="accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
</div>
<div class="carousel-item video-carousel-item">
<p class="carousel-caption">Ring Buffer Implementation in C</br>(Reading and Writing)</p>
<iframe width="500" height="315" src="https://www.youtube.com/embed/qynVLnXVOPg" frameborder="0" allow="accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
</div>
<div class="carousel-item video-carousel-item">
<p class="carousel-caption">Ring Buffer Implementation in C</br>(Initialization, Calculating Space)</p>
<iframe width="500" height="315" src="https://www.youtube.com/embed/QqenFBlPgXI" frameborder="0" allow="accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
</div>
</div>
</div>
</div>

